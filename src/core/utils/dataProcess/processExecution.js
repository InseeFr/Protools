...
export const claimTask = (user, taskID) => {
	...
	const apiUrl =
		window.env.REACT_APP_API_URL + urlEndpoint + user + '/' + taskID;
...
export const temporaryExecuteTask = (taskID, variables) => {
	const urlEndpoint = 'complete-task/';
	//const config = getConfigFile();
	const apiUrl = window.env.REACT_APP_API_URL + urlEndpoint + 'user/' + taskID;
	return fetcherPost(apiUrl)
		.then((r) => {
...
export const startProcess = (processKey, businessKey, variables) => {
	...
	//const config = getConfigFile();
	const apiUrl =
		window.env.REACT_APP_API_URL + urlEndpoint + processKey + '/' + businessKey;
	const dataUrl = [];
...
export const deleteProcess = (processID) => {
	...
	//const config = getConfigFile();
	const apiUrl = window.env.REACT_APP_API_URL + urlEndpoint + processID;
	return fetcherPost(apiUrl)
		.then((r) => {
