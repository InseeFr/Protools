...
const getHistoricActivity = async (
  ...
  historyResponseExec.data &&
    historyResponseExec.data.data.forEach((historicActivity: any) => {
      if (
        historicActivity.activityType === "userTask" ||
        historicActivity.activityType === "serviceTask" ||
        historicActivity.activityType === "subProcess"
      ) {
...
const getAllHistoricActivity = (
  ...
  return getRequest(
    ...
    res.data &&
      res.data.data.forEach((historicActivity: any) => {
...
const getAllHistoryProcessInstance = (
  ...
  return getRequest(
    ...
    res.data &&
      res.data.data.forEach((historicProcess: any) => {
        if (historicProcess.endTime) {
      ...
      res.data.data.forEach((element: any) => {
        variables.push({
          ...
          value: (() => {
            try {
              const parsedValue =
                typeof element.variable.value === "string"
                  ? JSON.parse(element.variable.value)
...
export const getHistoryUserActions = (
  ...
  return getRequest(
    ...
    Object.entries(variablesMap).forEach(([executionId, variables]) => {
      if (
        variables.some(
          (variable) => variable.name === "directory_access_pwd_contact",
        )
      ) {
        ...
        userCredentialsMap[executionId] = userCredentialsMap[
          executionId
        ].filter((userCredential) => userCredential.id !== "");
      }
    });
    const combinedUserCredentials: UserCredentials[] =
      Object.values(userCredentialsMap).flat();
